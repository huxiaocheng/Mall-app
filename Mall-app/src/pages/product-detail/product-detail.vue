  ...
  <transi-base name="slide">
    <div class="product-detail-wrap">
      ...
      >
        <div>
          <slider
            v-if="sliderDate && sliderDate.length > 0"
          ...
          <div class="product-info" ref="productInfo" v-if="Object.keys(productInfo).length > 0">
            <div class="price-wrap">
              ...
              <div class="original">￥{{productInfo.price / 0.8}}</div>
              <div class="sales">销量：{{Math.floor(productInfo.stock * Math.random())}}</div>
            </div>
            <p class="name">
              <span
                class="title-tips"
              >{{Math.random() > 0.3 ? '今日特价' : Math.random() > 0.6 ? '8折特惠' : '买2送1'}}</span>
              {{productInfo.name}}
            </p>
            <p class="sub-title">
...
export default {
  ...
  computed: {
    ...
    detailList() {
      if (Object.keys(this.productInfo).length > 0) {
        const reg = /http.+?(jpg|jpeg|png)/gi;
  ...
  methods: {
    ...
    addToCart() {
      addProductToCart(this.$route.params.id, this.cartCount)
        .then(res => {
          this.$Confirm({
            title: "添加成功, 是否去购物车查看？",
            cancelBtn: "返回",
  ...
  watch: {
    scrollY(newY) {
      ...
      if (
        -newY >
        this.$refs.productInfo.clientHeight + this.$refs.slider.$el.clientHeight
      ) {
        this.curIndex = 1;
      } else {
        this.curIndex = 0;
